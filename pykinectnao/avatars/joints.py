__author__ = 'Angeall'

#
#     Joints constants used to convert one skeleton into another skeleton
#     A Skeleton should contain a HashMap<String, Integer> to link the joints that the skeleton implements
#
#  --------------------------------------------------------------------------
#  KINECT FOR WINDOWS Standards / ! \ must remain in this order !
#
#      * The base of the spine, aligned with the hips
#
SPINE_BASE = 0

#
#      * The middle of the spine
#
SPINE_MID = 1

#
#      * The neck
#
NECK = 2

#
#      * The head
#
HEAD = 3

#
#      * The left shoulder
#
SHOULDER_LEFT = 4

#
#      * The left elbow
#
ELBOW_LEFT = 5

#
#      * The left wrist
#
WRIST_LEFT = 6

#
#      * The left hand
#
HAND_LEFT = 7

#
#      * The right shoulder
#
SHOULDER_RIGHT = 8

#
#      * The right elbow
#
ELBOW_RIGHT = 9

#
#      * The right wrist
#
WRIST_RIGHT = 10

#
#      * the right hand
#
HAND_RIGHT = 11

#
#      * the left hip
#
HIP_LEFT = 12

#
#      * the left knee
#
KNEE_LEFT = 13

#
#      * the left ankle
#
ANKLE_LEFT = 14

#
#      * The left foot
#
FOOT_LEFT = 15

#
#      * The right hip
#
HIP_RIGHT = 16

#
#      * The right knee
#
KNEE_RIGHT = 17

#
#      * The right ankle
#
ANKLE_RIGHT = 18

#
#      * The right foot
#
FOOT_RIGHT = 19

#
#      * The top of the spine, aligned with the shoulders
#
SPINE_SHOULDER = 20

#
#      * The tip of the left hand
#
HAND_TIP_LEFT = 21

#
#      * The left thumb
#
THUMB_LEFT = 22

#
#      * The tip of the right hand
#
HAND_TIP_RIGHT = 23

#
#      * The right thumb
#
THUMB_RIGHT = 24

#  --------------------------------------------------------------------------
#  OTHER possible joints (to be completed..)
#  --------------------------------------------------------------------------
#
#      * joint between the SPINE_BASE and the SPINE_MID
#
SPINE_LOW_MID = 25

#
#      * joint between the SPINE_MID and the SPINE_SHOULDER
#
SPINE_HIGH_MID = 26